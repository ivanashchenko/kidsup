...
h.append("<div class='q'>Что говорить тем, кто не дошёл: «Здравствуйте! Сегодня ждали [имя] на первом занятии — "
         "подскажите, занятия по [направление] ещё актуальны? … Неделя открытых уроков заканчивается на этой неделе, "
         "переживаю, чтобы вы успели попасть. Подберём удобную группу?» Если молчит второй раз — "
         "«Вы были записаны и не пришли, решила уточнить: всё ли у вас в порядке?»</div>")
h.append("<div class='card'><b>Правило разговора, который не закончился записью:</b> не «обращайтесь, когда решите», "
         "а «давайте я напишу вам в пятницу» — с датой, задачей в CRM и причиной следующего контакта. "
         "Отдельные ходы: сильно задумался — предложить альтернативный формат; заболел — сразу новое пробное; "
         "отказался — выяснить причину и дать альтернативу; не понравилось — приём «соцопрос»: "
         "«помогите нам стать лучше, что именно не подошло?»</div>")
...
h.append("</table></div>")
h.append("<div class='card warn'><b>Единая форма: что записать по каждому, пока человек не ушёл.</b> "
         "Не «поговорили — нормально», а девять полей. Без них завтрашний дожим делается вслепую:<br>"
         "<span class='small'>1. имя родителя · 2. имя и возраст ребёнка · 3. что интересует · 4. что попробовал · "
         "5. реакция ребёнка · 6. рекомендация педагога · 7. что предложили · 8. что ответил · 9. следующий шаг с датой</span><br>"
         "Пункты 5 и 6 берём у педагога до разговора с родителем — именно они превращают «подумаем» в оплату.</div>")
...
h.append("</table></div>")
h.append("<div class='card'><b>Три касания вместо одного «ну что, вы решили».</b> Каждое следующее сообщение "
         "должно давать новую причину вернуться к выбору, а не повторять вопрос.<br>"
         "<span class='small'><b>1. Напоминание:</b> «Вчера [имя] был у нас на первом занятии. Хотела вернуться к нашему разговору: "
         "сегодня ещё действует скидка 10% на абонемент. Прислать ссылку на оплату?»<br>"
         "<b>2. Полезное касание:</b> «Педагог отметил, что [имя] [конкретное наблюдение], но пока не хватает [что именно]. "
         "Отправляю рекомендации и программу группы, которую мы обсуждали. В группе осталось [N] мест — подержать за вами?»<br>"
         "<b>3. Забота:</b> «Вы не отвечаете на последние сообщения — переживаю, у вас всё в порядке?»</span></div>")
