...
h = [CSS, "<div class='wrap'>",
     ...
     "<p>Работаем 9:00–20:00 · Лена — дожим и оплаты · Ира — встреча, проводы и списки · Лиза — переписка<br>"
...
h.append("<div class='kpi'>"
         ...
         "</div>")
h.append("<div class='card warn'><b>Метрика живёт на отметке явки.</b> Педагог или Ира отмечают в CRM «пришёл» сразу после занятия — иначе семья не попадает ни в счётчик, ни в список дожима. "
...
grp("Не дошли и записаны заново", "не_дошли_записались",
    "Ира: подтвердить новую дату за день, спросить, что помешало в прошлый раз.", "p-amber")
grp("Не дошли и записи больше нет", "не_дошли",
    "Ира в окне 13:00–16:00. Не «почему не пришли», а следующий шаг на выбор: прийти на открытый урок на этой неделе, экспресс-консультация с педагогом, диагностика уровня или экскурсия по центру. Одно касание — одно конкретное предложение с днём и временем.", "p-red")
...
h.append("<div class='wcard lena'><div class='nm'>Лена — только дожим и оплаты</div><div class='rl'>Весь день одна задача: чтобы семья с пробного ушла оплатившей. Один дожим = 600 ₽ бонуса и 8 000 ₽ выручки</div>"
         ...
         "<b class='t'>16:00–20:00</b><span><b>Только разговоры с родителями на выходе.</b> Не встречает, не провожает, не отвечает на звонки — это Ира. Терминал и QR на стойке: оплата здесь и сейчас, ссылка «вдогонку» — только если родитель сам отказался платить на месте</span>"
         "</div></div>")
h.append("<div class='wcard anya'><div class='nm'>Ира — встреча, проводы и списки</div><div class='rl'>Телефон и дверь — её. К Лене родителей подводит, сама не продаёт</div>"
         "<div class='tl'>"
...
for t, d in [("1. Встреча и продажа — разные люди", "С 16:00 до 20:00 Ира встречает и провожает, Лена только разговаривает с родителями на выходе. Это её основная задача смены, а не «если успею»."),
...
h.append("<div class='card'>Ира утром: были или нет? Были — отметить «пришёл» и отдать Лене на дожим. Не были — перенести на конкретный день. "
...
h.append("<h2>Люди ждут ответа (Ира, с утра)</h2>" + task_table(waiting))
h.append("<h2>Поправить в CRM (Ира, до 13:00)</h2>" + task_table(crmfix))
h.append("<h2>Переносы, болезни, вопросы (Ира)</h2>" + task_table(moves))
