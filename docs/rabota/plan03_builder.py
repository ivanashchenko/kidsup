...
h.append("<div class='card alarm'><b>Три вещи, которые нельзя упустить сегодня.</b> "
         "1) " + L_ + " <b>Сорокина Полина</b> (79055389799) приходит с младшим на подготовку и планирует оплатить оба направления сразу — посчитать со скидкой 10% на второго ребёнка заранее, английский вт-чт 16:00 с 8.09 оформить. "
         "2) " + A_ + " встречает, " + L_ + " подтверждает утром — <b>Софья, 2 г. 5 мес.</b> (мама Юлия, 79104255162) — первое занятие в 12:00, раннее развитие с Ириной; записал я вчера сам, админы её не видели. "
         "3) " + L_ + " <b>Козлова Ангелина</b> (79687111684) со вчерашних 13:24 ждёт ответа: «есть ли группы обучения чтению?» — это ПШ2 читающие, и это мама двоих детей.</div>")
...
h.append("<div class='wcard' style='border-top-color:var(--blue)'><div class='nm'>Лиза — переписка</div><div class='rl'>Час днём, 15 минут с 16:00 до 20:00</div>"
         "<ol class='small'><li>" + Z_ + " Владимиру (Каранзин, 79998569433) — обещали стоимость раннего развития в WhatsApp: 7 800 за 8, 5 000 за 4, −10% в день первого занятия</li>"
         "<li>" + Z_ + " Сеничева (79265516487) — держать в заявках на танцы, ответить, как только будет расписание</li>"
         "<li>" + Z_ + " " + L_ + " " + A_ + " Никому не писать «это автоматическая рассылка, не обращайте внимания». Отвечать по существу: «да, это наше напоминание, ваша запись в силе»</li></ol></div>")
h.append("</div>")
...
h.append("<h2>Итог дня — записать в 20:00</h2><div class='card'><ul class='small'>"
         "<li>" + L_ + " Первое занятие: сколько было · оплатили на месте · назвали день · отказы и почему</li>"
         "<li>" + A_ + " Реактивация: сколько наборов из 40 · дозвонились · записали на пробное</li>"
         "<li>" + L_ + " Дожим: из 5 «пришли, не купили» — сколько оплатили</li>"
         "<li>" + A_ + " Отметка явки стоит у всех сегодняшних занятий — да/нет</li>"
         "<li>" + B_ + " Расписание робототехники — две группы набраны, админы ждут, чтобы обзвонить</li></ul></div>")
h.append("</div>")
