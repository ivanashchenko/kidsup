...
def spisok(since: str = OKNO_S, until: str = OKNO_PO, limit: int = 0) -> dict:
    ...
    with db.get_conn() as conn:
        ...
        if pokr.get("есть"):
            for ts, phone, direction, state in conn.execute(
                ...
                if day > d.get(p, ""):
                    d[p] = day
        # Манго пишет строку на каждое плечо звонка: один и тот же входящий
        # может лежать и как missed (на одном добавочном не сняли), и как
        # short (на другом сняли и сразу положили). Писать про такой звонок
        # «мы не сняли трубку» нельзя — сняли. Берём лучший исход: поговорили
        # важнее, чем сбросили, сбросили важнее, чем не сняли.
        for ph in list(ne_otvetili):
            if ph in sbrosili or ph in govorili:
                ne_otvetili.pop(ph)
    ...
    # Владелец 16.09, увидев страницу: «почему тут только номера телефонов?»
    # Потому что первая версия ставила наверх самое горячее — звонки, которые
    # мы не взяли, — а у них как раз имя и не заполнено: заполнять его было
    # некому, разговора не было. Страница открывалась списком голых номеров,
    # а живые фамилии уходили за кнопку. Сначала сортируем по тому, можно ли
    # по карточке работать, и только внутри — по горячести.
    _ORDER = {"можно звонить": 0, "имя не заполнено": 1,
              "не мобильный": 2, "служебная": 3}

    def _sort(f):
        return (_ORDER.get(f["качество"], 9), not f["не_ответили"],
                not f["сбросили"], not f["платил"],
                not bool(f["отвечал"]), f["создан"] or "")

    spisok_ = sorted(semyi.values(), key=_sort)
    if limit:
