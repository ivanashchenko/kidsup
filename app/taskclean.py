...
def _urgent_rewrite(body: str, created: str) -> str:
    ...
    if n <= 0:
        return body[:250]
    # Абсолютная дата, а не «3 дн. назад»: задача живёт дольше одного дня,
    # и относительный срок в ней устаревает молча. 21.08 восемь задач уехали
    ...
    if re.search(r"НОВАЯ ЗАЯВКА", body, re.I):
        return (f"📞 Заявка с сайта, оставлена {when} — позвонить. "
