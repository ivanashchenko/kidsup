...
def check() -> list:
    ...
    # Задачи, которые уже взял на себя Клод: их id лежат в очереди отправки.
    # Человеку их показывать не нужно — он потратит время на то, что
    # и так уйдёт сегодня.
    import json as _j
    mine_ids = {r.get("task_id") for r in
                _j.loads(db.get_setting("nabormail_queue", "[]") or "[]")
                if r.get("task_id")}
    ...
    for t in tasks:
        ...
        if t["id"] in mine_ids:
            row["verdict"] = "клод"
            row["why"] = "письмо клиенту уже в очереди у Клода"
        out.append(row)
    return out
...
GROUPS = [
    ("клод", "Взял на себя Клод — письма уже в очереди",
     "По этим задачам сообщение клиенту уходит автоматически, по одному "
     "раз в пару минут, и задача закроется сама по факту отправки. "
     "Человеку тут делать нечего."),
    ...
    ("актуальна", "Только человек — Клод сделать не может",
     "Деньги и документы, звонки голосом, организационное — то, что "
     "письмом не решается. Строки «КЛИЕНТ ЖДЁТ ОТВЕТА» разбирать "
...
def page(rows: list) -> str:
    ...
    out = [f"<style>{CSS}</style>",
           ...
           f"работы для человека остаётся {len(by['актуальна'])}, и из них "
