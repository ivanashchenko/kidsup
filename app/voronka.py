...
def trial_state(conn, uid: int, class_id: int | None) -> dict:
    ...
    if future:
        ...
        # Флаг «пробное» на записи ставит админ руками, а напоминание накануне
        # уходит только по записям с этим флагом (autopilot.trial_reminder
        # запрашивает lessonRecords с test=true). Нет флага — семья молчит.
        #
        # Но ругаться на каждую запись без флага нельзя: если ребёнка сразу
        # поставили на всю серию занятий группы, он уже не «пробник», а ученик,
        # и флаг ему не положен. 16.09 таких оказалось восемь из двадцати
        # девяти. Предупреждаем только про разовый визит — одна-две записи
        # в группе впереди.
        try:
            test = bool(json.loads(f["raw"] or "{}").get("test"))
        except (ValueError, TypeError, IndexError, KeyError):
            test = True          # не смогли прочитать — не пугаем админа зря
        series = sum(1 for r in future if r["class_id"] == f["class_id"])
        out = {"вид": "ждём", "дата": d, "время": t, "занятий_впереди": series,
               "флаг_пробного": test or series > 2,
