...
from .pult import _sklon as _sklon_
...
def _shag(kind: str, key: str, fn, imya: str = "") -> bool:
    """Шаг «раз в день/раз в час»: отметка ставится ТОЛЬКО после успеха.

    22.09.2026, аудит автопилота. Отметка ставилась в самом условии —
    `if _mark("money_day", …): money_check()`. Если проверка падала (кривая
    строка абонемента, МойКласс ответил 5xx), отметка уже стояла, ошибка
    уходила в лог одной строкой, и до завтра шаг не повторялся: за целый
    день ни одного пункта про неоплаченные абонементы. Никто об этом не
    узнавал — тихий пропуск выглядит точно так же, как «всё чисто».

    Теперь: сначала смотрим отметку, потом работаем, и метим только если
    получилось. Падение — видно владельцу в пульте, а не только в логе.
    """
    imya = imya or getattr(fn, "__name__", kind)
    if _has_mark(kind, key):
        return False
    try:
        fn()
    except Exception:
        log.exception("шаг «%s» упал — отметку не ставим, повторим на следующем тике", imya)
        # Раз в день говорим владельцу: шаг сегодня не отработал. Ключ с
        # пометкой «жалоба», чтобы не писать об этом каждые три минуты.
        if _mark(f"{kind}_zhaloba", key):
            try:
                inbox_add(f"🤖 Автоматика: шаг «{imya}» сегодня упал и не отработал. "
                          f"Пункты, которые он обычно приносит, за сегодня не появятся. "
                          f"Смотреть лог сервера за {_now().strftime('%H:%M')}.",
                          "", "Борис", "автоматика")
            except Exception:
                log.exception("не смог сообщить о падении шага «%s»", imya)
        return False
    _mark(kind, key)
    return True
...
# Скидки — одним числом на весь код. 22.09.2026, аудит: в подсказке для звонка
# стояло «скидка 10%», в задачах смены и в пунктах — 15%, на печатной табличке
# у стойки — 10%. Админ в один день читал оба числа, а по подготовке к школе
# это 420 ₽ с каждого абонемента и разговор «а нам написали другое».
# Правильные значения — решение владельца: 15% на ПЕРВЫЙ абонемент в день
# пробного, 8% на мини-сад и нулевой класс, 10% за второй предмет, второго
# ребёнка и многодетным. Скидки не суммируются — действует одна, наибольшая.
SKIDKA_PERVYJ = 15
SKIDKA_SAD = 8
SKIDKA_VTOROJ = 10


def _probnoe(napravlenie: str, zapas: int) -> str:
    """Цена пробного занятия — из прайса, а не числом в тексте.

    22.09.2026, аудит: в подсказке для звонка по каждому новому лиду стояло
    «робототехника 1 100 ₽», а в прайсе 1 600 ₽. Админ читал подсказку перед
    звонком и называл родителю цену на 500 ₽ ниже настоящей; на стойке
    выяснялось другое число. Цены живут в одном месте — PRICES.
    """
    try:
        from .main import PRICES
        for nazvanie, cena, _ in PRICES[napravlenie]["lines"]:
            if "робн" in nazvanie:                     # «Пробное занятие»
                return f"{int(cena):,}".replace(",", " ")
    except Exception:                                  # noqa: BLE001
        log.warning("цена пробного по «%s» не прочиталась из прайса", napravlenie)
    return f"{int(zapas):,}".replace(",", " ")


def _kogda_zanyatiya() -> str:
    """«занятия идут с 31.08» было верно в августе, а в подсказке осталось
    навсегда. Считаем от сегодняшней даты (22.09.2026, аудит)."""
    nachalo = date(2026, 8, 31)
    if _today() < nachalo:
        return f"занятия начинаются {nachalo.strftime('%d.%m')}"
    return "занятия уже идут, группа набрана — место освобождается редко"


def _akciya_stroka() -> str:
    """Акция «сентябрь по ценам прошлого года при оплате до 31.08» кончилась,
    а подсказка звала на неё ещё 22.09. Просрочённую акцию не называем."""
    konec = date(2026, 8, 31)
    if _today() <= konec:
        return (f"Акция: при оплате до {konec.strftime('%d.%m')} включительно сентябрь "
                f"идёт по ценам прошлого учебного года (продаём только помесячно). "
                f"ВАЖНО: старая цена и скидка НЕ складываются — считаем оба "
                f"варианта и предлагаем выгодный.")
    return (f"Скидок сверх обычных сейчас нет: {SKIDKA_PERVYJ}% на первый абонемент "
            f"в день пробного, {SKIDKA_SAD}% на мини-сад и нулевой класс, "
            f"{SKIDKA_VTOROJ}% за второй предмет, второго ребёнка и многодетным. "
            f"Скидки не суммируются — действует одна, наибольшая.")


def _akciya_korotko() -> str:
    """Одной фразой — что сейчас можно пообещать по деньгам. Августовская
    акция «сентябрь по старым ценам» кончилась 31.08, а тексты обзвона звали
    на неё ещё 22.09 (аудит)."""
    if _today() <= date(2026, 8, 31):
        return "До 31.08 сентябрь по старым ценам."
    return (f"При оплате первого абонемента в день первого занятия — "
            f"скидка {SKIDKA_PERVYJ}% (мини-сад и нулевой класс — {SKIDKA_SAD}%).")


def _nedelya_urokov() -> str:
    """Неделя открытых уроков шла 14–20.09. После неё звать на неё нельзя."""
    if _today() <= date(2026, 9, 20):
        return "Зови на Неделю открытых уроков 14–20.09."
    return "Зови на первое занятие своей группы — условно-бесплатное, с диагностикой."
...
def _manager_short(manager_id: int) -> str:
    """Короткое имя сотрудника по его id в МойКлассе: Аня, Лена, Ира, Лиза, Борис.

    22.09.2026, аудит пульта. Раньше имя искалось только среди дежурных
    администраторов (_admins), а Лизы и Бориса там нет. Пункт, адресованный
    Лизе (долги, закрытие занятий) или Борису (решение владельца), получал
    пустое имя — и inbox_add подставлял дежурную. То есть весь утренний
    разбор денег и расхождений падал Ане, у которой в этот день и без того
    полсотни своих дел, а колонка Лизы стояла пустой. Справочник имён лежит
    рядом (MANAGER_NAMES), достаточно в него заглянуть.
    """
    ...
    return MANAGER_NAMES.get(manager_id, "")


def obrezat(text: str, predel: int) -> str:
    """Длинный текст режем по границе предложения и ставим многоточие.

    22.09.2026, аудит пульта. Текст пункта резался молча ровно на пределе:
    из пятидесяти открытых пунктов двадцать четыре обрывались на полуслове.
    «…если не придут, снять запись, чтобы вре» — админ дочитывал до обрыва
    и не знал, что там было дальше; у владельца пункт обрывался ровно на
    вопросе, на который он должен был ответить. Обрыв должен быть видно.
    """
    t = (text or "").strip()
    if len(t) <= predel:
        return t
    kusok = t[:predel - 1]
    # ищем конец предложения, иначе — последний пробел
    granica = max(kusok.rfind(". "), kusok.rfind("! "), kusok.rfind("? "),
                  kusok.rfind("; "))
    if granica < predel * 0.6:
        granica = kusok.rfind(" ")
    if granica < predel * 0.5:
        granica = predel - 1
...
def inbox_add(text: str, phone: str = "", who: str = "", source: str = "автоматика") -> bool:
    ...
    with db.get_conn() as conn:
        ...
        # 22.09.2026, аудит. Дедуп по точному тексту был опасен в другую
        # сторону: тексты многих сигналов постоянны («Клиент не пришёл на
        # пробное — позвонить и перезаписать»), и когда в один вечер не
        # приходили трое детей, второй и третий пункт молча не создавались.
        # В логе при этом трижды стояло «задача → инбокс пульта». Поэтому
        # совпадение текста считаем дублем ТОЛЬКО когда совпадает и семья
        # (или телефона нет вовсе — тогда различить нечем).
        p10 = "".join(c for c in str(phone or "") if c.isdigit())[-10:]
        if len(p10) == 10:
            dup = conn.execute(
                "SELECT 1 FROM plan_inbox WHERE day=? AND text=? "
                "AND length(phone)>=10 AND substr(phone,-10)=?",
                (day, text, p10)).fetchone()
        else:
        ...
        # цифр, и кнопка «позвонить» на пульте собиралась как tel:+9032952727 —
        # телефон админа набирал Турцию, для 8123315033 Японию, для
        # 4956460344 Германию. На живой странице таких ссылок было 36.
        cifry = "".join(c for c in str(phone or "") if c.isdigit())[-11:]
        if len(cifry) == 10:
            cifry = "7" + cifry
        conn.execute("INSERT INTO plan_inbox (day, ts, who, text, phone, source) "
                     "VALUES (?,?,?,?,?,?)",
                     (day, _now().isoformat(timespec="minutes"), who, text, cifry, source))
    return True
...
def _task(mk: MoyklassClient, manager_id: int, user_id: int | None,
    ...
    if db.get_setting("crm_tasks_off", "0") == "1":
        ...
        # 22.09.2026, аудит: для владельца имя оставляли пустым, и пункт с
        # решением, которое может принять только он, ложился дежурной.
        who = "Борис" if manager_id == OWNER_ID else _manager_short(manager_id)
        if not inbox_add(body, phone, who, "автоматика"):
            # Молчаливый отказ прятал потерю: в логе стояло «задача → инбокс»,
            # а пункта не было ни у кого.
            log.warning("пункт НЕ создан (дубль по семье или пустой текст): %s | %s",
                        who or "дежурной", (body or "")[:70])
            return
...
def _hint_for_lead(mk: MoyklassClient, user_id: int, join: dict) -> None:
    ...
    lines = [
        ...
        f"2) Событие: {_kogda_zanyatiya()}. Первое занятие своей группы — "
        f"условно-бесплатное, с диагностикой; при оплате абонемента в этот же "
        f"день скидка {SKIDKA_PERVYJ}% на первый абонемент "
        f"(мини-сад и нулевой класс — {SKIDKA_SAD}%).",
        "3) Новые направления: «открываем танцы, хореографию, футбол, единоборства, акробатику, "
        f"актёрское мастерство, технику речи — что-то интересно? Запишу в лист ожидания, первой группе −{SKIDKA_VTOROJ}%».",
        f"Формулировка первого занятия — только «условно-бесплатное». Исключения: "
        f"ИЗО/шахматы — пробное {_probnoe('ИЗО', 850)} ₽ в зачёт абонемента, "
        f"танцы 850 ₽ если не остались, "
        f"робототехника {_probnoe('Робототехника', 1600)} ₽, если абонемент не куплен.",
        "",
...
# Слова, по которым отказ виден в самом сообщении, а не только в догадке
# модели. Без них статус «Отказ» не предлагаем — предлагаем уточнить.
_OTKAZ_SLOVA = re.compile(
    r"не будем|не пойд[её]м|не придём|не придем|отказ|передумал|"
    r"не ?актуальн|не стоит|не надо|больше не|в другом городе|переехал|"
    r"уже занимаемся|нашли друг|прекратить|уходим|не интересно", re.I)
...
def unanswered_inbound(mk: MoyklassClient) -> None:
    ...
    for phone, ts_in, chat_type, text in inbox:
        ...
        # 22.09.2026, аудит. «Спасибо большое! ❤️», «Хорошо, поняла», «Всё
        # получилось» — это закрытие разговора, а не вопрос. Такие сообщения
        # становились пунктом, а иногда и приказом поставить «Отказ».
        try:
            from .naryad import _polite as _vezhlivo
            if _vezhlivo(text):
                continue
        except Exception:
            pass
        ...
        if warm:
            ...
            body = (f"🔥 Ответил(а) на рассылку смайликом {text[:6]} ({ts_in[11:16]}, {who}) — "
                    f"тёплый. Спросить про планы на сентябрь, позвать на первое занятие, "
            ...
            # 22.09.2026, аудит. Догадка модели вклеивалась в пункт как
            # приказ, без пометки и без точки перед «Ответить». Живой пункт
            # 929: по картинке от Харьковских модель сочинила «Оформить
            # перевод остатка абонемента на Горина Федора» — такого решения
            # владелец не принимал, и админ пошла бы его исполнять. Теперь
            # догадка идёт отдельной фразой и честно названа догадкой, а по
            # картинке, голосовому и файлу её не берём вовсе: там читать
            # нечего.
            nechitaemo = text.strip().lower() in ("[image]", "[voice]", "[file]",
                                                  "[video]", "[sticker]", "")
            if sense and sense.get("следующий_шаг") and not nechitaemo:
                what = str(sense["следующий_шаг"]).rstrip(". ")
                body = (f"{head} ({ts_in[11:16]}, {who}): «{text[:70]}». "
                        f"Ответить {where}. Похоже на: {what} "
                        f"(догадка Клода — проверь по переписке).")
            else:
                body = (f"{head} ({ts_in[11:16]}, {who}): «{text[:90]}» — "
                        f"ответить {where} и поставить следующий шаг.")
            # 23.09.2026. Решение владельца 20.09: Лиза — только явка и долги.
            # Пункты «клиент писал, ответа нет» продолжали падать ей: за 23.09
            # их набралось 15, самопроверка пульта ловила их как «не тому», а
            # клиенты ждали. Переписку отвечает дежурная смены.
            owner = fallback
            # Отказ не требует ответа в чате — он требует пометки в карточке,
            # иначе человека продолжат звать и раздражать. Но ставить статус
            # по одной догадке модели нельзя: 22.09 на «Спасибо большое! ❤️»
            # пункт предлагал поставить «Отказ» и вычеркнуть семью из
            # обзвона. Нужен текстовый признак отказа в самом сообщении.
            if sense and sense.get("намерение") == "отказ" \
                    and not sense.get("ждёт_ответа") and not nechitaemo:
                if _OTKAZ_SLOVA.search(text):
                    body = (f"Отказ ({ts_in[11:16]}, {who}): «{text[:80]}» — "
                            f"поставить статус «Отказ» с причиной и не звонить.")
                else:
                    body = (f"Похоже на отказ ({ts_in[11:16]}, {who}): «{text[:80]}» — "
                            f"уточни у семьи и только потом ставь статус. "
                            f"Слов отказа в сообщении нет, это догадка Клода.")
        _task(mk, owner, uid, obrezat(body, 250))
...
def trial_reminder(mk: MoyklassClient) -> None:
    ...
    for r in recs:
        ...
        _wa(phone, f"Здравствуйте! Это KidsUP на Бульваре Рокоссовского 🌿\n"
                   ...
                   f"Как найти: ориентир — магазин «Дикси», от него по лестнице наверх, "
                   f"вход со стороны бульвара. Не сориентируетесь — позвоните, встретим.\n"
    ...
    for r in recs:
        ...
        if admins:
            _task(mk, admins[0]["managerId"], uid,
                  ...
                  "Причина решить сейчас: скидка 15% на первый абонемент действует только в день пробного (сад и НК — 8%).")
        phone = user.get("phone")
        if phone:
            _wa(phone, f"Спасибо, что пришли к нам сегодня! 🌿 Если хотите, перешлём "
                       f"комментарий педагога о том, как {child} занимался.\n"
                       f"Решите продолжить — скидка 15% на первый абонемент действует "
    ...
    for num, times in missed.items():
        ...
        if mobile or uid:
            ...
            _task(mk, admins[0]["managerId"], None,
                  f"Городской +7{num} звонил {_sklon_(times, 'раз', 'раза', 'раз')} "
                  f"и не дозвонился. "
...
MISSED_COLD = (
    ...
    "Занятия уже начались — можно прийти на первое занятие своей группы:\n"
    "📚 оно условно-бесплатное и с диагностикой: педагог смотрит уровень "
    "ребёнка и подбирает группу\n"
    "🎁 при оформлении первого абонемента в день первого занятия — скидка 15%\n"
...
MISSED_OUR = (
    ...
    "на 2026/27 учебный год: он идёт с 31 августа, войти можно в любой "
    "момент — педагог на первом занятии смотрит уровень и дальше "
    "подтягивает индивидуально.\n"
...
def card_quality() -> None:
    ...
    try:
        ...
        _task(mk, who, None, obrezat(body, 250))
...
def morning_tasks(mk: MoyklassClient) -> None:
    ...
    TEXT_HOT = ("🔥 Был у нас ЭТИМ ЛЕТОМ. Начни с «как вам лето у нас» — он "
                "помнит педагога. Цель звонка: привести на первое занятие своей группы "
                "(условно-бесплатное, с диагностикой). В подсказке 🎯 — "
    ...
    TEXT_WARM = ("Занимался у нас до апреля-мая — не ушёл, а закончил сезон. "
                 ...
                 "При оплате первого абонемента в день первого занятия — скидка 15%.")
    TEXT_RR = ("Раннее развитие: малыш подрос — предложи группу нового года "
               "по возрасту (в подсказке 🎯). Свободных мест в РР больше всего. "
               f"{_nedelya_urokov()}")
    # 22.09.2026, аудит: во всех трёх текстах админ читал «до 31.08 сентябрь
    # по старым ценам» и звал на Неделю открытых уроков 14–20.09 — и то, и
    # другое кончилось. Акцию и событие подставляем по дате, а не буквой.
    TEXT_CONTIN = ("Продолжение занятий 2026/27: в подсказке 🎯 — чем занимался "
                   ...
                   f"каллиграфию, скорочтение, менталку. {_akciya_korotko()}")
    TEXT_CAMP = ("🔥 Летний лагерь закончился — продающий звонок про учебный год: "
                 ...
                 "плюс направления по возрасту». Зови на "
                 "первое занятие своей группы — условно-бесплатное, с диагностикой; "
                 f"{_akciya_korotko()}")
    TEXT_RENEW = ("Продление: семья занимается у нас летом (МсМ/логопед/сад). "
                  ...
                  f"по возрасту из подсказки 🎯. {_akciya_korotko()}")
        ...
        for uid in queues.get(idx, []):
            ...
            try:
                ...
                if not _mark("call_task", str(uid)):
                    ...
                    if retry is None:
                        continue
                    # 22.09.2026, аудит: пункт приходил без имени ребёнка и
                    # без темы — «Недозвон, повтор №2», и всё. Админ не знала
                    # ни кому звонит, ни о чём говорить, и шла искать карточку.
                    _imya = (user.get("name") or "").strip()[:32] or "имя не заполнено"
                    _tel_ = "".join(c for c in str(user.get("phone") or "") if c.isdigit())[-10:]
                    _tema = {"contin": "продолжение занятий", "camp": "был в лагере",
                             "regular": "продление с сентября", "hot": "был у нас этим летом",
                             "warm": "занимался до весны", "rr": "раннее развитие"}.get(kind, "набор 2026/27")
                    _task(mk, adm["managerId"], uid,
                          f"Недозвон, попытка №{retry}: {_imya}"
                          + (f" +7{_tel_}" if _tel_ else "")
                          + f" — {_tema}. Позвонить в другое время дня "
                            f"(утром, если прошлый раз звонили вечером). "
                            f"После разговора поставить статус.")
                    made += 1
                    continue
...
def auto_reject(mk: MoyklassClient) -> None:
    ...
    for uid, phone in users:
        ...
        if not p or p in answered or counts.get(p, 0) < limit:
            continue
        if _has_mark("auto_reject", str(uid)):
            continue
        # 22.09.2026, аудит. Статус «Отказ» ставился без причины, а МойКласс
        # на такой запрос отвечает 400 (причины клиентских статусов — свой
        # справочник, из общего ряда принимается только 313613, см.
        # app/otkaz.py:248). Отметка при этом ставилась ДО запроса: карточка
        # считалась обработанной навсегда и не повторялась никогда. То есть
        # ни один клиент этим шагом в «Отказ» так и не переведён.
        try:
            mk.post(f"/v1/company/users/{uid}/status",
                    {"statusId": ST_REJECT, "statusChangeReasonId": 313613})
        except Exception as e:  # noqa: BLE001
            log.warning("auto_reject: статус карточке %s не поставился (%s) — "
                        "отметку не ставим, повторим завтра", uid, e)
            continue
        _mark("auto_reject", str(uid))
    ...
    try:
        if unsigned:
            ...
            for a in (_admins_today() or [])[:2]:
                _task(mk, a["managerId"], None, obrezat(body, 250))
        if refunds:
...
RULE_TASK = {
    "freeze-offbook": "Заморозку надо ставить полями «заморозить с/по» в абонементе, "
                      "а не словами в комментарии. Иначе три недели за год не "
                      "посчитать, а со стороны это выглядит как скидка от себя.",
    "freeze-over": "У клиента заморозка сверх трёх недель за год. Сверх нормы — "
...
def rules_check() -> dict:
    ...
    try:
        for mgr, items in by_mgr.items():
            ...
            # Разбор ошибок администраторов — владельцу. У звонящих
            # администраторов в задачах должны быть ТОЛЬКО звонки и сообщения
            # ради записи детей (решение Бориса 21.08), а с 20.09 у Лизы
            # остались только закрытие занятий и долги — правила посещения
            # не её работа. Раньше это шло Лизе, а поскольку её имя не
            # находилось, падало дежурной: 22.09 Аня получила четыре пункта
            # бухгалтерии поверх пятидесяти своих.
            from .pult import _sklon
            who = MANAGER_NAMES.get(mgr, f"менеджер {mgr}")
            body = (f"🤖 Клод: правила посещения, оформил {who} — "
                    f"{_sklon(len(items), 'расхождение', 'расхождения', 'расхождений')}. "
                    f"{what}. {hint}")
            try:
                _task(mk, OWNER_ID, items[0].get("user_id"), obrezat(body, 250))
            except Exception:
...
def money_check() -> dict:
    ...
    try:
        for mgr, items in by_mgr.items():
            ...
            from .pult import _sklon
            who = MANAGER_NAMES.get(mgr, f"менеджер {mgr}")
            body = (f"🔥 Клод: деньги в CRM, оформил {who} — "
                    f"{_sklon(total, 'расхождение', 'расхождения', 'расхождений')}. "
                    f"{what}. Абонемент без оплаты продолжает "
...
NO1_TEXT_V2 = (
    ...
    "особенное — занятия уже начались, "
    "беспроигрышной лотереей в парке «Янтарная горка» (у ЖК Богородский) "
    "первое занятие условно-бесплатное с диагностикой, "
...
ENG_PS = (
    ...
    "Cambridge YLE с сертификатом. На Неделе открытых уроков (14–20.09) "
...
def _loop() -> None:
    ...
    while True:
        try:
            ...
            if time.monotonic() - last3 >= 180:
                ...
                try:
                    ...
                    if 9 <= now.hour <= 20 and _mark("slot_q15_missed_in", _q15):
                        missed_inbound(mk)
                    # Чаты групп английского: кто уже входил в личный кабинет,
                    # но ещё не в чате своей группы, — добавляем сами. Раз в
                    # день рано утром: каждый ребёнок это заход в браузер на
                    # минуту, днём браузер нужен для других задач.
                    # 21.09 Борис: «всех кто добавляется в ТвойКласс и ученик
                    # АЯ — сразу добавлять в соответствующий чат МойЧат».
                    if now.hour == 7 and _mark("chaty_sinhron_day", str(_today())):
                        try:
                            from . import chaty
                            chaty.snapshot()          # сперва свежий состав чатов
            ...
            if now.hour >= 9:
                _shag("audit_day", str(_today()), audit_yesterday, "аудит вчерашнего дня")
            ...
            if (now.hour, now.minute) >= (9, 30):
                _shag("rules_day", str(_today()), rules_check, "контроль правил")
            # деньги в CRM: неоплаченные абонементы, минусовые балансы,
            # бесплатные отметки и несписанные пропуски — в 9:40, следом
            # за контролем правил
            if (now.hour, now.minute) >= (9, 40):
                _shag("money_day", str(_today()), money_check, "проверка денег")
            ...
            if (now.hour, now.minute) >= (9, 50):
                def _fit_day():
                    ...
                    if n:
                        ...
                        try:
                            if True:
                                # Состав групп — возраст, уровень, перебор —
                                # это решение владельца, а не работа
                                # переписчика: кого переводить и какую группу
                                # закрывать, решает Борис (22.09, аудит).
                                _task(mk, OWNER_ID, None,
                            ...
                            mk.close()
                _shag("fit_day", str(_today()), _fit_day, "проверка состава групп")
            if now.hour >= 8:
                _shag("close_dead_tasks", str(_today()), close_dead_tasks,
                      "чистка задач по архивным клиентам")
            if now.hour >= 20:
                _shag("digest", str(_today()), daily_digest, "вечерняя сводка")
