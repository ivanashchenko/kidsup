    ...
    while True:
        try:
            ...
            if time.monotonic() - last3 >= 180:
                ...
                # Рассылка по набору в Telegram и MAX. Зовём каждую минуту,
                # но темп внутри задаёт сам модуль: одно письмо и случайная
                # пауза 1–2,5 минуты. Ровный залп из сорока сообщений — это
                # подпись бота, за которую Telegram и MAX снимают аккаунт;
                # владелец остановил такую отправку 25.08. Идёт, пока WABA
                # на модерации — потом второй заход шаблоном по молчунам.
                if 9 <= now.hour < 20:
                    try:
