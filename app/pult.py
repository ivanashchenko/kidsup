...
HOWTO = ("<div class='card' style='border-left:4px solid #312783;margin:10px 0;font-size:15px'>"
         ...
         "<li><b>Личный кабинет «Твой Класс» и чаты английского — не твоя работа.</b> Проверяет, кто вошёл, добавляет детей в чаты групп и пишет в поддержку Клод. Твоя часть одна: если у семьи в карточке нет почты или она с ошибкой — спросить почту и вписать в поле «Email». Родителю, который не может войти, можно сказать: «kidsup.tvoyklass.com → ваша почта → «Восстановить пароль» → письмо от lk-noreply@tvoyklass.com, часто в «Спаме»». Не вышло — пункт Борису, дальше не разбирайся.</li>"
         "</ul></details></div>")
