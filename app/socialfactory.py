...
PRICE_DEADLINE = date(2026, 8, 30)   # до этой даты сентябрь по ценам года

CENTER = ("детский центр и английский сад KidsUP, б-р Маршала Рокоссовского 6к1В "
          "(напротив ТЦ «Янтарь», 2 минуты от м. Бульвар Рокоссовского)")
...
def _subject(name: str) -> str | None:
    ...
    if re.search(r"_РР|^РР|аннее развит|ицей", n, re.I):
...
def post_price_deadline(f: dict, on: date | None = None) -> dict | None:
    ...
    return {
        "рубрика": "Продажа",
        "заголовок": f"До 30 августа сентябрь по ценам прошлого года ({when})",
        "текст": (
            "Мы держим прошлогоднюю цену на сентябрь для тех, кто определится "
            "до 30 августа. С 31-го начинается учебный год и действуют новые "
            "цены.\n\n"
            "Скидки, которые есть у нас:\n"
            "· −15% на первый абонемент, если оформить в день пробного занятия "
    ...
    if on == HOLIDAY:
        return {"рубрика": "Событие", "заголовок": "Сегодня праздник открытия сезона",
                "текст": ("Сегодня ждём вас на празднике открытия сезона! "
                          ...
                          "Рокоссовского, 6к1В — напротив ТЦ «Янтарь», две "
                          "минуты от метро. До встречи!")}
    if on == OPEN_DOORS:
