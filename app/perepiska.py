...
ADDRESS = ("б-р Маршала Рокоссовского, 6к1В (напротив ТЦ «Янтарь»), "
           "5 минут от метро Бульвар Рокоссовского")
CHAT_ADMIN = 154181
...
# Признаки вопроса или просьбы: без них совпавшее слово — не запрос.
ASK_RE = re.compile(r"\?|подскаж|скажите|пришли|прислать|отправьте|скиньте|"
                    r"сколько|какое|какие|каком|когда|где\s|есть\s+ли|можно\s+ли|"
                    r"интересу|хотел[аи]?\s+бы\s+узнать", re.I)
...
def scan(day: str | None = None) -> list[dict]:
    ...
    for d in _dialogs(day):
        ...
        # 17.09, Лоева: «сейчас пытаюсь выстроить график» — не вопрос, а
        # ответ администратору посреди разговора; слово «график» дало тему
        # «расписание», и семье ушёл полный список групп английского, а
        # администратор дописала «это ИИ отправил». Отвечаем сами только
        # на то, что сформулировано как вопрос или просьба.
        if topic and not ASK_RE.search(last["text"] or ""):
            topic = ""
        if not subj:
