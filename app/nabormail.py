...
DATES = ("29 августа — праздник открытия сезона\n"
         ...
         "31 августа – 13 сентября — неделя открытых уроков")
...
def text_for(name: str, seg: str, paid: bool) -> str:
    ...
    # в карточке вместо имени бывает телефон или пустая строка — тогда
    # «79629193433 занимался у нас» ушло бы клиенту как есть
    if child and (len(child) < 2 or any(c.isdigit() for c in child)):
        child = ""
    if paid:
...
def _child_for(name: str) -> str:
    """«для Виктории», «для Льва» — или «для вашего ребёнка», если имени нет.

    25.08 предпросмотр показал сразу две беды: «место для Виктория»
    (именительный падеж вместо родительного) и «место для 79629193433» —
    в карточке вместо имени стоял телефон, и он честно подставился
    в письмо. Оба случая ловятся здесь, до отправки."""
    from .autopilot import _genitive
    child = _first_name(name)
    # имя должно быть именем: буквы, не короче двух знаков, без цифр
    if not child or len(child) < 2 or any(c.isdigit() for c in child):
        return "для вашего ребёнка"
    if not any("а" <= c.lower() <= "я" for c in child):
        return f"для {child}"            # латиница — не склоняем
    return f"для {_genitive(child)}"


def push_text(name: str, seg: str) -> str:
    """Второе касание: человеку уже писали, и он не ответил.

    Повторять то же самое нельзя — это читается как «вы не ответили».
    Поэтому вместо напоминания о себе даём то, чего в первом письме
    не было: конкретную группу с днями и временем, куда ребёнку можно
    прийти прямо сейчас. И явное разрешение не отвечать — без него
    второе письмо превращается в давление."""
    who = _child_for(name)
    place = {
        "1-3": "раннее развитие, утренние группы — вторник и четверг "
               "в 10:00, 11:00 и 12:00, есть воскресные",
        "3-5": "«Лицей для малышей» — среда и пятница, 16:00 или 17:00",
        "5-7": "подготовка к школе — будни с 16:00 до 19:00, "
               "есть субботние группы",
        "7-12": "английский по уровням Cambridge — понедельник-среда "
                "или вторник-четверг, вечернее время",
        "?": "группы по возрасту — от раннего развития до английского "
             "и подготовки к школе",
    }.get(seg, "группы по возрасту")
    ...
    try:
        ...
        for uid, titles in by_user.items():
            ...
            tail = (f"{when_start} Всё как обычно — б-р "
                    ...
                    "Рокоссовского, 6к1В (напротив ТЦ «Янтарь»), 5 минут "
