...
· ЛИЗА (liza) — переписка в мессенджерах и оплаты клиентов: счета,
  ссылки на оплату, возвраты, ответы в чатах.
· ДЕЖУРНЫЙ (duty) — звонки, запись в группы, подтверждение приходов.
...
def _plan(row: dict) -> dict | None:
    ...
    if who in ("owner", "liza", "duty"):
        want = {"owner": OWNER, "liza": CHAT_ADMIN, "duty": None}[who]
        if who == "duty":
