...
def rows() -> list[dict]:
    with db.get_conn() as conn:
        ...
        for c in cls:
            ...
            out.append({"id": c["id"], "name": short, "cap": cap, "live": live, "free": max(cap - live, 0),
                        "merge": note, "wait": wait})
        def key(r):
...
def block() -> str:
    ...
    def td(r):
        ...
        return (f"<tr><td style='font-size:13px'>{html.escape(r['name'])}</td>"
                f"<td class='num' style='white-space:nowrap'>{r['live']} / {r['cap']}</td><td style='font-size:13px'>{st}</td></tr>")
    ...
    return (f"<details class='card' style='border-left:4px solid #7DB928;margin:14px 0'>"
            f"<summary style='cursor:pointer;font-size:17px;font-weight:800'>Свободные места сейчас: {free_total} в {len(rs)} группах, живых записей {live_total}</summary>"
            ...
            f"Красным — группы, куда записываем первыми; «не записывать» — сливаются 14–18.09, вместо них соседняя группа.</div>"
            f"<div class='scroll'><table><tr><th>Группа</th><th class='num'>живых / норма</th><th>места</th></tr>"
