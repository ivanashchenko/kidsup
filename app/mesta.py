...
MERGE = {  # подстрока имени группы -> куда вместо
    # 22.09.2026: прежнее решение по «АЯ_вт-чт_16:00» («с 22.09 → Starters 5–8
    # в этом слоте, детей 8–12 в Гр1») отменено владельцем. Группа остаётся
    # на месте и становится 9–13 лет: из четверых ходящих под рамку 10–12
    # проходила одна, а так не выпадает никто.
...
WAITLIST = {  # переполненные: не дописывать, лист новой группы
    # 22.09.2026, решение владельца и админов по английскому:
    "АЯ_пн-ср_18:00": "ПОЛНАЯ, запись закрыта — нулевым 6–7 лет предлагать вт-чт 18:00 (Гр7)",
    "АЯ_вт-чт_19:00": "полная — лист на вторую продвинутую группу 8–11",
...
HOLD = {
    # 22.09.2026, Ира: «группа 9–13 лет педагогически неверная. То, что
    # девятилетки легли к девочкам 11–13, — случайность: уровень одинаковый,
    # но это разные классы. Записывать третьеклассника вместе с семилеткой
    # нельзя. Нужно уточнять класс, а не возраст». Пока классы не собраны и
    # владелец не решил, что делать с составом, в эту группу не записываем.
    "АЯ_вт-чт_16:00": "пауза: состав собран по уровню, а классы разные — ждём классы детей и решение Бориса",
