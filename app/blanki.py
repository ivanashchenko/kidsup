...
SUBJ_ORDER = ["подготовка к школе", "английский", "раннее развитие",
              "ментальная арифметика", "ИЗО", "шахматы",
              "нулевой класс", "мини-сад", "логопед"]

CONTACTS = [
    ("Адрес", "б-р Маршала Рокоссовского, 6 к1В — напротив ТЦ «Янтарь», "
              "5 минут пешком от м. Бульвар Рокоссовского"),
    ("Сайт", "kidsup.ru"),
    ...
    ("События", "14–20.09 — Неделя открытых уроков: приходите на любое "
                "занятие по расписанию, первое условно-бесплатное"),
]
...
@media print{{
  ...
  /* На печати воздух между группами — это лишние страницы: при исходных
     отступах помещалось четыре группы на лист, и весь набор занимал 27.
     Строки под запись остаются достаточно высокими, чтобы вписать имя
     от руки. */
  .grp{{padding:.3rem .45rem .35rem;margin-bottom:.35rem}}
  .gh{{padding-bottom:.2rem;margin-bottom:.25rem}}
  .gh b{{font-size:.8rem}}
  .row{{height:1.15rem}}
  .rows{{gap:0 .7rem}}
  .sh{{margin-bottom:.35rem}}
...
<div class="note"><b>Как пользоваться.</b> Группы идут по предметам и внутри
предмета по номерам: Гр1, Гр2, Гр3 — так их называют между собой и в разговоре
с родителем. У каждой группы свои дни, время и восемь мест: подписанные —
дети, которые уже записаны, пустые — свободные. Родитель называет удобный
день, вы смотрите по группам этого предмета, куда он попадает.

Записали ребёнка карандашом в свободную строку, после разговора передали
в центр: имя, возраст, телефон, предмет и группу. Пока запись не внесена
в систему, места за ребёнком нет.</div>
