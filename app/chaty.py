...
def text_for(groups_: list[dict]) -> str:
    ...
    if all(g.get("in_chat") for g in groups_):
        # уже добавлены админом — звать «напишите, добавим» нельзя
        return (head + f"Вы уже в чате: он откроется в личном кабинете "
                f"МойКласс — {LK_URL}, заходите по номеру телефона, на "
                f"который оформлен ребёнок.\n\nЕсли чат не видно — "
                f"напишите нам, поможем.")
    return (head + f"Чат открывается в личном кабинете МойКласс: {LK_URL} — "
            f"заходите по номеру телефона, на который оформлен ребёнок.\n\n"
