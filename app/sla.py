...
def verify_closed() -> dict:
    ...
    10.09.2026, по разбору Иры: «нет звонка и сообщения» — не то же самое,
    что «ничего не сделано». Вопрос решают и на ресепшене голосом, и в
    предыдущем звонке; след такой работы — комментарий в карточке, а не
    новый звонок. Проверка переоткрывала задачу с укором «закрыта без
    действия», и админ получал выговор за уже сделанную работу. Раз владелец
    отключил задачи в CRM (crm_tasks_off=1), надзирать за ними мы больше не
    вправе — задачи, которые админ завёл себе сам, это его личный список."""
    from . import db as _db
    if _db.get_setting("crm_tasks_off", "0") == "1":
        return {"reopened": 0, "skipped": "crm_tasks_off"}
    mk = _client()
    reopened = 0
    try:
