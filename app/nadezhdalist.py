...
# Берём только тех, кто был у нас начиная с прошлого сентября: прошлый
# учебный год и это лето. Дальше память о центре слабеет, и разговор
# перестаёт отличаться от холодного.
RECENT_FROM = "2025-09-01"
LAST_YEAR = re.compile(r"^2526_")
SUMMER_26 = re.compile(r"2526_ЛК|^2626_|лето 2026", re.I)
...
# Названия групп бывают и с префиксом года («2024_ПШ_…»), и без него
# («ПШ_Индивидуальное 1_сб_13:00»). Шаблоны, требовавшие подчёркивания
# впереди, вторые пропускали: у семьи в листе стояло «мини-сад», хотя
# ребёнок ходил ещё и на подготовку.
SUBJ = [("подготовка к школе", r"(^|_)ПШ|подготовк"), ("английский", r"(^|_)АЯ|английск"),
        ("мини-сад", r"(^|_)МС|МсМ|мини-сад|детский сад"), ("нулевой класс", r"(^|_)НК|нулев"),
        ("лагерь", r"(^|_)ЛК|лагер|летний клуб"), ("ИЗО", r"(^|_)ИЗО"),
        ("шахматы", r"(^|_)ШХ|ШАХ|шахмат"), ("ментальная арифметика", r"(^|_)МА|ментальн"),
        ("раннее развитие", r"(^|_)РР|раннее|музыка и речь"),
        ("скорочтение", r"(^|_)СЧ|скорочт"), ("логопед", r"(^|_)ЛГ|логопед")]
...
IS_PS = re.compile(r"ПШ|подготовк|нулев", re.I)


def _teacher(group: str) -> str | None:
    """Кто вёл эту группу подготовки к школе.

    Правило владельца: все группы вт-чт и нулевой класс вела Инга,
    остальные группы ПШ — Катя. Имя педагога в разговоре решает многое:
    «вы ходили к Инге» звучит как «мы вас помним», а не как обзвон
    по списку. Обе сейчас у нас не работают — тем важнее, чтобы
    звонящий об этом знал заранее и не обещал того же педагога."""
    n = group or ""
    if not IS_PS.search(n):
        return None
    # «Заявки» — буфер, куда кладут до распределения в группу. Ребёнок там
    # у педагога не занимался, и говорить «вы ходили к Инге» было бы неправдой.
    if re.search(r"аявк", n, re.I):
        return None
    if re.search(r"нулев", n, re.I):
        return "Инга"
    if re.search(r"вт\s*[-–]\s*чт", n, re.I):
        return "Инга"
    return "Катя"
...
def _fake_phone(digits: str) -> bool:
    ...
    Их заводят, когда номер не спросили, а карточку сохранить надо.
    В листе обзвона такая строка — потерянная минута и раздражение."""
    tail = digits[-10:]
    if len(set(tail)) <= 1:          # 9999999999 и подобные
        return True
    # Пороже нельзя: у настоящих номеров вроде 9161116111 всего две разные
    # цифры, и жадный фильтр выбросил бы живого клиента.
    return tail in {"1234567890", "0000000000", "9876543210", "1111111111"}
...
def collect() -> list[dict]:
    ...
    for j in joins:
        ...
        if s and j.get("userId"):
            was[j["userId"]].add(s)
        t = _teacher(nm)
        if t and j.get("userId"):
            teach[j["userId"]].add(t)
    # Берём дату НАЧАЛА абонемента: конец у действующего лежит в будущем,
    # и в колонке «когда были» стояло бы «2026-10» у семьи, которая ходила
    # весной. Начало отвечает на вопрос «когда ребёнок у нас занимался».
    last = defaultdict(str)      # когда ходил — по началу абонемента
    seen_until = defaultdict(str)  # до каких пор действовал — для отбора
    for s in subs:
        uid = s.get("userId")
        if not uid:
            continue
        b, e = (s.get("beginDate") or "")[:10], (s.get("endDate") or "")[:10]
        if b > last[uid]:
            last[uid] = b
        if max(b, e) > seen_until[uid]:
            seen_until[uid] = max(b, e)

    # Кто был у нас начиная с прошлого сентября. Два источника, потому что
    # порознь каждый неполон: абонемент бывает не заведён, а в группе ребёнок
    # числится — и наоборот. Вместе дают 163 семьи против 132 по одним
    # абонементам, то есть треть нашлась бы только вторым способом.
    # Отбираем по последнему дню действия абонемента: годовой, купленный
    # весной 2025, означает, что ребёнок ходил весь прошлый учебный год,
    # хотя дата начала лежит раньше отсечки.
    ...
    for u in users:
        ...
        if len(phone) < 11 or _fake_phone(phone):
            continue
        age = _age(u)
        if age is None or not (AGE_LO <= age <= AGE_HI):
            continue
        # Только те, кто был у нас в прошлом учебном году или этим летом.
        # Холодная часть базы (946 семей, никогда у нас не занимавшихся)
        # для обзвона не годится: там разговор начинается с нуля, а у нас
        ...
        out.append({
            ...
            "was": sorted(was.get(u["id"], set()))[:4],
            "teacher": ", ".join(sorted(teach.get(u["id"], set()))),
            "last": last.get(u["id"], ""),
            "busy": u["id"] in busy,
        })
    out.sort(key=_order)
    ...
    for a in sorted(by_age):
        ...
        for c in items:
            ...
            trs.append(
                ...
                f"<td class='tch'>{H.escape(c.get('teacher') or '—')}</td>"
...
<div class="note"><b>Эти семьи центр знают.</b> Ребёнок у нас занимался,
родитель платил и помнит педагогов. Разговор начинается не с «здравствуйте,
мы детский центр», а с «вы у нас занимались, а на новый год ещё не
записаны» — в колонке «что было у нас» видно, что именно предлагать.</div>
