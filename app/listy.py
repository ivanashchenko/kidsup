...
def build() -> dict:
    ...
    # Решение владельца 25.08: делим базу не по педагогу, а по свежести
    # денег. Ира добивает лето 2026 — там ребёнок был у нас месяц назад;
    # Лена берёт прошлый учебный год, где сидят готовые кандидаты в ПШ и
    # английский, — именно эти группы горят сильнее всего (ПШ 28/120,
    # английский 17/64).
    ...
    for slug, title, sub, rows, cols in (
        ...
        # Админ Клуба Буракова с 25.08 обзвон не ведёт (решение владельца),
        # но лист продолжаем собирать: если решение отменят, он должен быть
        # свежим, а не собранным месяц назад.
