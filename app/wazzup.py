...
def guard(phone: str, text: str, kind: str = "",
    ...
    # 26.09.2026: в карточке «Кушнир Полина» стоял номер самого центра
    # (79160170918), и реактивация ушла из WhatsApp центра в него же —
    # Борис увидел её у себя в Wazzup. Свои номера адресатом не бывают.
    svoi = {WABA_SENDER[-10:], CHAT_SENDER[-10:], "9165610077"}
    for key in ("chat_whatsapp", "waba_sender"):
        v = "".join(c for c in str(db.get_setting(key, "") or "") if c.isdigit())[-10:]
        if v:
            svoi.add(v)
    if p in svoi:
        return f"+7{p} — номер самого центра, в карточке клиента он стоит по ошибке"
...
# 28.08 19:00: WhatsApp на 0077 снова заблокирован Meta (пятый бан) —
# вся переписка и рассылки идут с 0918, единственного живого канала.
CHAT_SENDER = "79160170918"
