...
30-го и Неделю открытых уроков 31.08–13.09, но вести родителя было некуда:
...
# Неделя продлена до 13.09 (решение владельца 06.09): день называем
# ближайшим будущим числом, чтобы «среда» не оказалась прошедшей.
def _week() -> dict:
    ...
    for i, k in enumerate(DAYS):
        ...
        while d < today and d + timedelta(days=7) <= date(2026, 9, 13):
            d += timedelta(days=7)
...
EVENTS = [
    ...
    {"when": "31 августа — 13 сентября", "tag": "Неделя открытых уроков",
...
13 сентября в детском центре KidsUP на бульваре Рокоссовского.">
<style>
...
<p class="lead">С 31 августа по 13 сентября занятия идут по обычному
