...
Зачем. В рассылках мы зовём на Неделю открытых уроков 14–20.09, но вести родителя было некуда:
...
Что на странице. Событие с датой, временем и тем, что будет. Ниже живое расписание открытых уроков из CRM: родитель
...
# Вторая Неделя открытых уроков — 14–20 сентября 2026 (решение владельца
# 13.09). Проверено по календарю: понедельник — 14 сентября, воскресенье —
# 20-е. День называем ближайшим будущим числом, чтобы «среда» не оказалась
# прошедшей.
def _week() -> dict:
    from datetime import date, timedelta
    names = ["понедельник", "вторник", "среда", "четверг", "пятница", "суббота", "воскресенье"]
    months = {8: "августа", 9: "сентября"}
    today = date.today()
    out = {}
    for i, k in enumerate(DAYS):
        d = date(2026, 9, 14) + timedelta(days=i)
        while d < today and d + timedelta(days=7) <= date(2026, 9, 20):
            d += timedelta(days=7)
        out[k] = f"{names[i]} {d.day} {months.get(d.month, '')}"
    return out
WEEK = _week()

EVENTS = [
    # Августовские события (праздник 29.08, День открытых дверей 30.08)
    # убраны 13.09: прошли. Осталась вторая Неделя открытых уроков.
    {"when": "14 — 20 сентября", "tag": "Неделя открытых уроков",
     "title": "Настоящее занятие, а не показательное",
     "text": "Всю неделю можно прийти на любой урок по расписанию. "
             "Ребёнок занимается вместе с группой, родитель смотрит, "
             "как идёт занятие и как работает педагог.",
...
<title>KidsUP — Неделя открытых уроков 14–20 сентября</title>
<meta name="description" content="Неделя открытых уроков 14–20 сентября
в детском центре KidsUP на бульваре Рокоссовского: настоящее занятие
своей группы, первое — условно-бесплатное.">
<style>
...
<p class="lead">С 14 по 20 сентября занятия идут по обычному
